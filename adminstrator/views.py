...
def freight_view(request):
    ...
    if fform.is_valid():
        ...
        newrate = freightRate(Source=source, Destination=desti, Port_freight=means,
                              From_source_to_Port_freight=sourcemeans, From_port_to_destination_freight=desmeans,
                              metric=metric, MiniQuantitySent=minsent, MaxQuantitySent=maxsent,
                              MiniQuantityRecieved=minrecieve, MaxQuantityRecieved=maxrecieve, Total_cost=tcost,
                              Unit_cost=ucost, Sourcetax=stax, Destinationtax=dtax, Sourceothertax=sotax,
                              Destinationothertax=dotax, MarkupRate=markup, Product_types=ptype, DestinAgent=destagent,
                              OriginAgent=orgagent, )
        newrate.save()
...
def product_view(request):
    ...
    context = {'user': Client.objects.filter(user=request.user), 'username': request.user.username,
               'flintproducts': flintproducts.objects.filter(Active=True), 'user_name': currentUser.username,
               'times': timezone.now(), 'bioproducts': bioproducts.objects.filter(Active=True),
               'btproducts': btproducts.objects.filter(Active=True)}
...
def create_admin(request):
    ...
    if form.is_valid():
        ...
        if user is not None:
            ...
            if user is not None:
                currentUser = request.user
                new_client = Client(user=currentUser, physical_address=physical_address, role=role, privilege=priv,
                                    Country=country, phonenumber=phone, Alternate_phonenumber=altphone, WeChat=wechat,
                                    Skype=skype)
                new_client.save()
...
def verify_product(request):
    ...
    context = {'user': Client.objects.filter(user=request.user),
               'flintproducts': flintproducts.objects.filter(Active=False), 'user_name': currentUser.username,
               'times': timezone.now(), 'bioproducts': bioproducts.objects.filter(Active=False),
               'btproducts': btproducts.objects.filter(Active=False)}
...
def verify(request, comp, pk):
    if comp == "Mineral":
        ...
        bt = bioproducts.objects.get(pid=pk)
        bt.Active = True
        bt.save(update_fields=['Active'])
        return redirect('/adminstrator/Verifyproduct/')
...
def emailer(request):
    ...
    smtpObj.starttls()
    email = 'btproducts4u@gmail.com'
    password = 'Roseda@2018bt'
